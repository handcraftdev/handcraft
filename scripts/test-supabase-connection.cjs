...
// Usage: node scripts/test-supabase-connection.cjs
...
function loadEnvFile() {
  ...
  envContent.split('\n').forEach(line => {
    ...
    if (equalsPos > 0) {
      ...
      // Extract just the URL part if there are comments
...
async function main() {
  ...
  if (supabaseUrl) {
    ...
    const commentPos = supabaseUrl.indexOf('#');
    if (commentPos > 0) {
      supabaseUrl = supabaseUrl.slice(0, commentPos).trim();
    }
  }
