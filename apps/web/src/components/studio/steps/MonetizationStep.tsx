...
export function MonetizationStep({ draft, onUpdate, onNext, isEditMode = false }: MonetizationStepProps) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h2 className="text-2xl font-medium text-white/90 mb-2">Monetization</h2>
      <p className="text-white/40 mb-8">Set pricing for buying and renting your content</p>

      {/* Buy Price Section */}
      <div className="p-5 rounded-2xl border border-purple-500/20 bg-purple-500/5 mb-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-medium text-white/90">Buy Price *</h3>
        </div>

        <div className="space-y-4">
          <div>
            <label className="block text-sm text-white/50 mb-2">Price in SOL (min 0.001)</label>
            <input
              type="number"
              step="0.001"
              min="0.001"
          ...
          <div>
            <label className="block text-sm text-white/50 mb-2">Supply Limit (max 999,999)</label>
            <input
              type="number"
              min="1"
              max="999999"
      ...
      <div className="p-5 rounded-2xl border border-amber-500/20 bg-amber-500/5 mb-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-medium text-white/90">Rental Prices *</h3>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <input
              ...
              placeholder="0.005"
              className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white/90 text-sm focus:outline-none focus:border-amber-500/50"
            />
          </div>
          <div>
      ...
      {/* Visibility Section */}
      <div className={`p-5 rounded-2xl border mb-6 ${isEditMode ? 'border-white/5 bg-white/[0.01]' : 'border-white/10 bg-white/[0.02]'}`}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-medium text-white/90">Visibility</h3>
      ...
      <button
        onClick={onNext}
        disabled={!canProceed}
        className="w-full py-3 bg-purple-500/20 hover:bg-purple-500/30 disabled:opacity-30 disabled:cursor-not-allowed rounded-xl font-medium transition-all border border-purple-500/30 text-white/90"
      >
        Continue to Review
      </button>
    </div>
