              ...
              <div className="flex flex-col items-center gap-3">
                ...
                {!publicKey && (
                  <button
                    ...
                    className="px-6 py-2.5 bg-white text-black rounded-full text-sm font-medium hover:bg-white/90 transition-colors flex items-center gap-2"
                  >
                ...
                {/* Visibility Level 2: Members */}
                {publicKey && content.visibilityLevel === 2 && (
                  <Link
                    href={`/profile/${content.creatorAddress}`}
                    className="px-6 py-2.5 bg-purple-500 hover:bg-purple-600 text-white rounded-full text-sm font-medium transition-colors flex items-center gap-2"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
                    </svg>
                ...
                {/* Visibility Level 3: Buy/Rent (or when wallet connected and level 0/undefined with mint/rent config) */}
                {publicKey && (content.visibilityLevel === 3 || content.visibilityLevel === undefined || content.visibilityLevel === 0) && (
                  <div className="flex justify-center gap-3">
                    {(bundleContext ? hasBundleMintConfig : hasMintConfig) && (
                      <button
                        onClick={() => bundleContext ? setShowBuyBundleModal(true) : setShowBuyContentModal(true)}
                        className="px-6 py-2.5 bg-white text-black rounded-full text-sm font-medium hover:bg-white/90 transition-colors"
                      >
                        Buy {bundleContext ? (bundleMintPrice && `· ${Number(bundleMintPrice) / 1e9} SOL`) : (mintPrice && `· ${Number(mintPrice) / 1e9} SOL`)}
                      </button>
                    )}
                    {(bundleContext ? hasBundleRentConfig : hasRentConfig) && (
                      <button
                        onClick={() => bundleContext ? setShowRentBundleModal(true) : setShowRentModal(true)}
                        className="px-6 py-2.5 bg-white/10 text-white rounded-full text-sm font-medium hover:bg-white/20 transition-colors border border-white/20"
                      >
                        Rent {bundleContext ? (bundleLowestRentPrice && `· ${bundleLowestRentPrice / 1e9} SOL`) : (lowestRentPrice && `· ${lowestRentPrice / 1e9} SOL`)}
                      </button>
                    )}
                  </div>
                )}
